...
class PDCFRPlus(_CFRBase):
    ...
    def weight(self, T):
        d, w = np.power(T - 1, self.alpha) / (np.power(T - 1, self.alpha) + 1), 1
        return d, w
    ...
    def _regret_formula_after_first_it(self, ev_all_actions, strat_ev, last_regrets):
        ...
        d, w = self.weight(T)
        regrets = np.maximum(regrets * d + imm_regrets * w, 0)
        return regrets

    def _regret_formula_first_it(self, ev_all_actions, strat_ev):
        T = self._iter_counter + 1
        d, w = self.weight(T)
        return np.maximum((ev_all_actions - strat_ev) * w, 0)
        ...
        for t_idx in range(len(self._trees)):
            ...
            def _fill(_node):
                if _node.p_id_acting_next == p_id:
                    ...
                    regrets = _node.data["regret"]
                    imm_regrets = _node.data["imm_regret"]
                    d, w = self.weight(T + 1)
                    _reg = np.maximum(regrets * d + imm_regrets * w, 0)
