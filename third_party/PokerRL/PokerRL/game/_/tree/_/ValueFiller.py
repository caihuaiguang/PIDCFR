...
fold_matrix_dict = None
equity_matrix_dict = None
...
class ValueFiller:
    def __init__(self, tree):
        ...
        self._eq_const = self._env_bldr.rules.N_CARDS_IN_DECK / (
            self._env_bldr.rules.N_CARDS_IN_DECK - 1
        )
    ...
    def compute_cf_values_heads_up(self, node):
        ...
        if node.is_terminal:
            ...
            # Check / Call
                ...
                if node.env_state[EnvDictIdxs.current_round] == Poker.FLOP:
                    equity = self._get_call_eq_final_street(
            ...
            # set boardcards to 0
            for c in self._env_bldr.lut_holder.get_1d_cards(
                node.env_state[EnvDictIdxs.board_2d]
            ):
            ...
            ev_all_actions = np.zeros(
                shape=(N_ACTIONS, self._tree.n_seats, self._env_bldr.rules.RANGE_SIZE),
                dtype=np.float32,
            )
            ev_br_all_actions = np.zeros(
                shape=(N_ACTIONS, self._tree.n_seats, self._env_bldr.rules.RANGE_SIZE),
                dtype=np.float32,
            )
            ...
            if node.p_id_acting_next == self._tree.CHANCE_ID:
                ...
                node.ev = np.zeros(
                    shape=(self._tree.n_seats, self._env_bldr.rules.RANGE_SIZE),
                    dtype=np.float32,
                )
                node.ev_br = np.zeros(
                    shape=(self._tree.n_seats, self._env_bldr.rules.RANGE_SIZE),
                ...
                node.ev[plyr] = np.sum(
                    node.strategy.T * ev_all_actions[:, plyr], axis=0
                )
                ...
                node.br_a_idx_in_child_arr_for_each_hand = np.argmax(
                    ev_br_all_actions[:, plyr], axis=0
                )
    ...
    def _get_call_eq_final_street(self, reach_probs, board_2d):
        ...
        for h in range(self._env_bldr.rules.RANGE_SIZE):
            handranks[h] = self._env.get_hand_rank(
                board_2d=board_2d,
                hand_2d=self._env_bldr.lut_holder.get_2d_hole_cards_from_range_idx(
                    range_idx=h
                ),
            )
    ...
    def _get_call_eq_preflop(self, node):
        ...
        for c in range(self._env_bldr.rules.N_CARDS_IN_DECK):
            ...
            equity += self._get_call_eq_final_street(
                reach_probs=_reach_probs, board_2d=_board_2d
            )
...
class ValueFillerHUNL(ValueFiller):
            ...
            for p1_card2 in range(p1_card1 + 1, 52):
                ...
                for p2_card1 in range(52):
                    for p2_card2 in range(p2_card1 + 1, 52):
                        ...
                        if (
                            p1_card1 == p2_card1
                            or p1_card1 == p2_card2
                            or p1_card2 == p2_card1
                            or p1_card2 == p2_card2
                        ):
                            out[p1_idx, p2_idx] = 0
                            out[p2_idx, p1_idx] = 0
    ...
    def set_board(self, node):
        ...
        if (
            hash_board_1d in self.fold_matrix_dict
            and hash_board_1d in self.equity_matrix_dict
        ):
        ...
        if round == Poker.RIVER:
            ...
            for card_1d in range(52):
                if card_1d not in board_1d:
                    ...
                    new_board_1d = list(
                        self._env_bldr.lut_holder.get_1d_cards(new_board_2d)
                    )
    ...
    def load_matrix_dict(self):
        ...
        if not file.exists():
            ...
            global fold_matrix_dict
            global equity_matrix_dict
            if fold_matrix_dict is None:
                with open(self.root_path / "fold_matrix_dict.pkl", "rb") as f:
                    fold_matrix_dict = pickle.load(f)
            if equity_matrix_dict is None:
                with open(self.root_path / "equity_matrix_dict.pkl", "rb") as f:
                    equity_matrix_dict = pickle.load(f)
            self.fold_matrix_dict = fold_matrix_dict
            self.equity_matrix_dict = equity_matrix_dict
    ...
    def evaluate_board(self, board_2d):
        ...
        for h in range(self._env_bldr.rules.RANGE_SIZE):
            hand_1d = list(
                self._env_bldr.lut_holder.get_1d_cards(
                    self._env_bldr.lut_holder.get_2d_hole_cards_from_range_idx(
                        range_idx=h
                    )
                )
            )
